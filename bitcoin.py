...
def similmeasure(a, b):
    num=0
    denom=0
    n=len(a)
    res=0
    ma, mb = np.mean(a), np.mean(b)
    stda, stdb = np.std(a), np.std(b)
    for i in range(0, n):
        num = (a[i]-ma)*(b[i]-mb)
        denom = float(n*stda*stdb)
        res = num/denom
...
def computeDelta(wt, X, Xi):
    ...
    for i in range(len(mat1)):
        num +=  (mat1[i][n] * math.exp(wt * similmeasure(X[0:n], mat1[i][0:n])))
        denom += math.exp(wt * similmeasure(X[0:n], mat1[i][0:n]))
    E_emp = float(num/denom)
    return E_emp
