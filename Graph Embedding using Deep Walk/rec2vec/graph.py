...
def records_to_graph():
    ...
    for m in movies:
        ...
        id = id+1
        for r in range(1,6):
            n = Node(id, m+"_"+str(r), 'movie-rating')
            nodedict[m+"_"+str(r)] = n
            nodelist.append(n)
            id = id+1
    ...
    # YOUR CODE HERE
    for u, r in ratings.iteritems():
        for m, t in r.iteritems():
        
            unode = nodedict[u]
            rnode = nodedict[m+'_'+t]
            mnode = nodedict[m] 
            unode.neighbors.append(rnode)
            rnode.neighbors.append(unode)
            mnode.neighbors.append(rnode)
            rnode.neighbors.append(mnode)
            
    for m, t in movies.iteritems():
        mnode = nodedict[m]
        if(movies[m].director != None):
        ...
        for ma in movieactors:
            anode = nodedict[ma]
            mnode.neighbors.append(anode)
            anode.neighbors.append(mnode)
            
        moviegenres = t.genres
        for mg in moviegenres:
            gnode = nodedict[mg]
            mnode.neighbors.append(gnode)
